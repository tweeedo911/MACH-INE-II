...
// ── Shimmer: arpeggio delle note dell'accordo corrente in registro alto ──
function updateShimmer(dt) {
  if (presence[1] < 0.2) return;
  ...
  notes.forEach((note, i) => {
    setTimeout(() => {
      ...
      const vel = Math.floor(35 + presence[1] * 30);
      sendMIDINote(5, note, vel, beatMs * sustain * 2);
      addMidiNote(5, note / 127, vel / 127);
    }, i * Math.round(beatMs * 0.8));
  });
}
...
function onBeat(beat) {
  ...
  // CH1 GRAIN — rare sparkles, high register
  if (presence[2] > 0.3 && beat % 4 === 0 && Math.random() < presence[2] * 0.4) {
    const scale = MODES5[currentMode];
    const hi   = scale.filter(n => n >= 79 && n <= 91);
    const pool = hi.length > 0 ? hi : scale;
    const note = pool[Math.floor(Math.random() * pool.length)];
    const vel  = Math.floor(25 + presence[2] * 30);
    sendMIDINote(1, note, vel, beatMs * sustain);
