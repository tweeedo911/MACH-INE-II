...
let _lastNote = -1;
let _velSweep = 0;
let _lastFollowChord = '';  // for "follow harmony" mode — detect chord changes
...
function _tick() {
  ...
  const pattern = track.bassPattern;   // 16-step array or null
  ...
  if (!pattern) {
    ...
    if (chordStr && chordStr !== _lastFollowChord && _step === 0) {
      ...
      if (bassNote < regLo || bassNote > regHi) bassNote = root; // fallback
      ...
      const dur = Math.round(beatMs * 14); // long sustain — almost 4 bars
  ...
  // ═══ Mode B: Pattern-based — syncopated groove ═══
