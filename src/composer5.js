...
function updateRupture() {
  ...
  if (ruptureStage !== lastRuptureStage) {
    ...
    if (ruptureStage === 'presagio') _ruptureZone5 = 0;
  }
}
...
// ── Rupture register Markov — crystal shattering escalates upward ──
const _RUPTURE_ZONES5 = [
  MODES5.F_locrian.filter(n => n < 64),              // zone 0: low  (53–63)
  MODES5.F_locrian.filter(n => n >= 64 && n < 76),   // zone 1: mid  (65–75)
  MODES5.F_locrian.filter(n => n >= 76),              // zone 2: high (77–87)
];
const _RUPTURE_MATRIX5 = [
  [0.1, 0.6, 0.3],  // low  → tende verso mid/high
  [0.2, 0.2, 0.6],  // mid  → tende verso high
  [0.3, 0.4, 0.3],  // high → può tornare (frammentazione diffusa)
];
let _ruptureZone5 = 1;
function _ruptureNextZone5() {
  const row = _RUPTURE_MATRIX5[_ruptureZone5];
  let r = Math.random();
  for (let z = 0; z < row.length; z++) { r -= row[z]; if (r <= 0) { _ruptureZone5 = z; return; } }
  _ruptureZone5 = row.length - 1;
}

// ── Shimmer: variable arpeggio patterns from chord in high register ──
function updateShimmer(dt) {
  if (presence[1] < 0.15) return;
  if (getPresenceMultiplier('cristallo') <= 0.5) return;
...
function onBeat(beat) {
  ...
  // CH7 RUPTURE — cluster crash (crystal shattering, zone-based escalation)
  if (ruptureStage !== 'idle' && ruptureStage !== 'residuo') {
    if (ruptureStage === 'takeover' && beat % 2 === 0) {
      // Cluster: fire many notes simultaneously from current register zone
      const pool = _RUPTURE_ZONES5[_ruptureZone5];
